...
(function() {
  ...
  // ─── WeChat Settings ───
  C.initWechatSettings = function() {
    const loginBtn = C.elements.wechatLoginBtn;
    const statusText = C.elements.wechatStatusText;
    const qrContainer = C.elements.wechatQRContainer;
    const qrImage = C.elements.wechatQRImage;
    const wechatToggle = C.elements.wechatToggle;

    if (!loginBtn) return;

    function updateUI(status, detail) {
      C.state.wechatStatus = status;
      const colors = { disconnected: '#999', connecting: '#FFB347', connected: '#4CAF50', error: '#FF453A' };
      const labels = {
        disconnected: '📱 微信桥接：未连接',
        connecting: '📱 微信桥接：连接中...',
        connected: '📱 微信桥接：' + (detail || '已连接'),
        error: '📱 微信桥接：' + (detail || '错误'),
      };
      statusText.textContent = labels[status] || labels.disconnected;
      statusText.style.color = colors[status] || colors.disconnected;
      if (status === 'connected') wechatToggle.checked = true;
      else if (status === 'disconnected') wechatToggle.checked = false;
    }

    window.petAPI.onWechatQR(function(dataUrl) {
      qrImage.src = dataUrl;
      qrContainer.style.display = 'block';
    });

    window.petAPI.onWechatStatus(function(data) {
      if (data && data.status) {
        updateUI(data.status, data.detail);
    ...
    window.petAPI.onWechatLoginResult(function(data) {
      qrContainer.style.display = 'none';
      if (data.success) {
        updateUI('connected', '已登录');
      } else {
        updateUI('error', data.error || '登录失败');
      }
    });

    loginBtn.addEventListener('click', async function() {
      if (C.state.wechatStatus === 'connecting') return;
      updateUI('connecting', '正在获取二维码...');
      var result = await window.petAPI.wechatLogin();
      if (!result.success) {
        updateUI('error', result.error || '登录失败');
      }
    });

    if (wechatToggle) {
      window.petAPI.wechatGetStatus().then(function(st) {
        if (st.loggedIn && st.bridgeRunning) { updateUI('connected'); wechatToggle.checked = true; }
        else if (st.loggedIn) { updateUI('disconnected'); wechatToggle.checked = false; }
      });

      wechatToggle.addEventListener('change', async function() {
        var en = wechatToggle.checked;
        var r = await window.petAPI.wechatToggle(en);
        if (r.success) {
          if (en) {
            var st = await window.petAPI.wechatGetStatus();
            if (st.loggedIn) {
              updateUI('connected');
            } else {
              updateUI('connecting', '请先扫码登录');
            }
          } else {
